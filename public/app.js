...
const Lobby = {
    ...
    async createWheel() {
        ...
        if (!fee || fee < 1) {
            toast("Enter a valid entry fee", "error");
            return;
        }
...
class SpinWheelEngine {
    ...
    spinToIndex(targetIndex, duration = 4500) {
        return new Promise((resolve) => {
            ...
            const targetNorm =
                (-Math.PI / 2 - (((targetIndex + 0.5) * arc) % (2 * Math.PI)) + 4 * Math.PI) % (2 * Math.PI);
...
const Admin = {
    async init() {
        await Promise.all([Admin.loadConfig(), Admin.loadActiveWheel(), Admin.loadUsers()]);
    },
